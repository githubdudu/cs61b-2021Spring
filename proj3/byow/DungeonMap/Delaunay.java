...
public class Delaunay {
    ...
    public static Set<Triangle> bowyerWatson(List<Point2D> pointList) {
        ...
        Triangle superTriangle = new Triangle(new Point(-GraphUtils.HEIGHT, 0),
                new Point(GraphUtils.WIDTH / 2, GraphUtils.HEIGHT + GraphUtils.WIDTH / 2),
                new Point(GraphUtils.WIDTH + GraphUtils.HEIGHT, 0));
        ...
        for (Point2D p : pointList) { // add all the points one at a time to the triangulation
            ...
            for (Triangle.Edge e : polygon) { // re-triangulate the polygonal hole
                triangulation.add(new Triangle(e.getP1(), e.getP2(), p));
            }
        }
        // Remove triangles that shared edges with "super triangle"
